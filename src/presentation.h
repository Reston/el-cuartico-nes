...
void episode_progress(u8 y){
 center(y,remix?"REMIX     0/3 SELLOS":"EPISODIO  0/3 SELLOS");addr(0x2000+((u16)y<<5)+16);
...
void help_screen(void){u8 i;
 ...
 if(paused&&host==0&&task_active){
  ...
  center(12,titles[host]);for(i=0;i<5;++i)center(14+i*2,help_lines[host][i]);
 }
...
void pause_screen(void){
 slate();ui_heading("PAUSA",3);center(12,titles[host]);
...
void pause_close(void){
 ...
 paused=0;pause_help=0;music_restore=1;hud_update();hud_on=pause_hud;on();
}
void result_screen(void){u8 n,total;u16 errors;
 ...
 n=host==0?repairs:(host==1?hits:found);total=host==0?12:(host==1?20:3);
 print(5,18,host==0?"REPARACIONES":(host==1?"ACIERTOS":"ENCUENTROS"));ui_count(20,18,n);print(22,18,"/");ui_count(23,18,total);
