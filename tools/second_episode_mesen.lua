...
local function launch(host)
 ...
 if r('episode')==1 then
  ...
  press('a')
 elseif host==2 then assert(r('mode')==7);press('a')end
...
local runner=coroutine.create(function()
 ...
 for episode=0,1 do
  ...
  for _,host in ipairs({0,2,1})do
   launch(host)
   local trace,acts={},{};local did_pause=false
   for f=1,17000 do
    if r('mode')<1 or r('mode')>3 then break end
    if r('hud_on')==0 then frames(1)
    else
     if episode==1 and not did_pause and ((host==0 and r('task_active')==1)or(host==2 and r('prop_mask')==1)or(host==1 and r('hold_slot')<3))then
   ...
   if host==1 then check(r('hits')==(episode==1 and 49 or 20),'Rhythm objective is correct for episode '..(episode+1))end
